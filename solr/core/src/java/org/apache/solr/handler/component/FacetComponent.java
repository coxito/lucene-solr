 ...
 // Distributed facet_pivots
  ...
  for(Entry<String, PivotFacetWrapper> pfwEntry : fi.pivotFacets){
    if (pfwEntry.getValue().shardPivotFacets.size() > 0) {
      ...
      WritePivotFacetCollections(rb, pfwEntry.getValue());
    }
  ...
  private void WritePivotFacetCollections(ResponseBuilder rb, PivotFacetWrapper pfw) {
  ...
  private void processTopElement(PivotFacetWrapper pfw, LinkedHashMap<String, Comparable> currentFullPivotPathList, int facetLimit) {
    ...
    for (Entry<Integer, Map<Comparable, NamedList<Object>>> shardsPivotFacets : pfw.shardPivotFacets.entrySet()) {
      ...
      if (pivotResultsFromShard.size() > 0
          && !PivotFacetHelper.doesSpecifiedLocationExist(pivotResultsFromShard, currentFullPivotPathList)
  ...
  private void refinePivotFacets(ResponseBuilder rb, ShardRequest sreq) {
    ...
    for (ShardResponse srsp : sreq.responses) {
      ...
      for (Entry<String, List<NamedList<Object>>> singleFacetPivotResponse : response_facet_pivots) {
        // This is each Facet.Pivot response
        PivotFacetWrapper singleFacetWrapper = fi.pivotFacets.get(singleFacetPivotResponse.getKey());
        singleFacetWrapper.shardRefinementPaths.clear();
        Map<Comparable, NamedList<Object>> specificshardresult = singleFacetWrapper.shardPivotFacets.get(shardNum);
        // Fields list is the key
        for (NamedList<Object> incomingPivotFacet : singleFacetPivotResponse.getValue()) {
          // Each field within this response (probably just one)
          int numberOfPivots = 1 + StringUtils.countMatches(singleFacetPivotResponse.getKey(), ",");
          // Fields list is the key
          ...
          if (specificshardresult.containsKey(pivotFacetsValue)) {
            ...
            mergePivotFacetShardMaps(
                PivotFacetHelper.getMappedPivots(theResultThatWillBeUpdated),
                PivotFacetHelper.getMappedPivots(incomingPivotFacet),1, numberOfPivots);
          } else {
            // Gotten from the shard response, is lists down below,
            // not mapps, convert to merge properly with mapped
            // shard results
            if (incomingPivotFacet.indexOf(PivotListEntry.PIVOT.getName(), 0) > 0) {
              PivotFacetHelper.setMappedPivotsFromListPivots(incomingPivotFacet, numberOfPivots);
              PivotFacetHelper.remove(PivotListEntry.PIVOT,incomingPivotFacet);
    ...
    if (refinementComplete) { // All refinement requests have been received, we delete them when they come back
      for(Entry<String, PivotFacetWrapper> pfwEntry : fi.pivotFacets){
          WritePivotFacetCollections(rb, pfwEntry.getValue());
          queuePivotRefinementRequests(rb,pfwEntry.getValue());
      }
  ...
  public void finishStage(ResponseBuilder rb) {
    ...
    facet_counts.add("facet_pivot", combinedPivotFacets);
  }
